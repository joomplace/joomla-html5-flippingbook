...
class HTML5FlippingBookControllerPages extends JControllerAdmin
{
	...
	public function convert()
	{
		...
		$movedPDFFileFullName = $pdfDirName . '/' . $pdfFileName;
		$tempPDFFileFullName = $tempDirName . '/' . $pdfFileName;

		if (!JFile::move($pdfFile['tmp_name'], $movedPDFFileFullName) || !JPath::setPermissions($movedPDFFileFullName))
		{
		...
		if (class_exists('Imagick'))
		{
			...
			{         
				$output_big = $tempDirName . "/" . $fName . "-" . $i . ".jpg";
		...
		{
			// Determine num of pages
			$num_pages = (int)$this->_getPDFPages($tempPDFFileFullName);
		...
		$this->setRedirect('index.php?option='.COMPONENT_OPTION.'&view=pages', JText::_('COM_HTML5FLIPPINGBOOK_BE_PAGES_CREATED'));
	}
