...
$sortFields = array(
	'template_name' => JText::_('COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_NAME'),
...
<form name="adminForm" id="adminForm" action="<?php echo 'index.php?option='.COMPONENT_OPTION.'&view='.$this->getName(); ?>" method="post" autocomplete="off">
	...
	<div id="j-main-container" class="span10 html5fb_templates">
		...
		<table class="table table-striped html5fb_table html5fb_templates_table">
			<thead>
				<tr>
					<th>
					...
					<th width="100%">
						<?php echo JHtml::_('grid.sort', 'COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_NAME', 'template_name', $listDirn, $listOrder); ?>
					</th>
					<th width="1%">
				...
				if (count($this->items) > 0)
				{
					foreach ($this->items as $i => $item)
					{
						...
						<tr class="row<?php echo $i % 2; ?>">
							...
							<td>
								<a href="<?php echo JRoute::_('index.php?option='.COMPONENT_OPTION.'&task=template.edit&id='.$item->id); ?>"><?php
									echo $this->escape($item->template_name); ?></a>
							</td>
							<td width="1%">
								<?php echo $item->id; ?>
							</td>
			...
			<tfoot>
				...
				if (count($this->items) == 0)
				{
					...
					{
						$html[] = JText::_('COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_NOITEMS') . ' – ' .
							'<a onclick="javascript:Joomla.submitbutton(\'template.add\')" href="javascript:void(0);">' .
