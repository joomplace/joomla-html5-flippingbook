...
abstract class HTML5FlippingBookFrontHelper
{
	...
	public static function htmlPublHelper($mobile = FALSE, $tablet = FALSE, $item='', $link = FALSE, $fullPath = FALSE)
	{
		...
		{
			$data->publicationLink = JRoute::_($data->rawPublicationLink. ($item->uid == $user->get('id') && ($item->page) ? '#page/' . $item->page : ''), FALSE, (int)$uri->isSSL());
		...
		if ($download->convert || $download->cloudconvert || $download->c_enable_pdf)
		{
			...
			if($download->c_enable_pdf || $formats){
				if($download->c_enable_pdf){
					$content .= '
								<li>
									<a href="' . JRoute::_('index.php?option='.COMPONENT_OPTION .'&task=convert.getpdf' . '&id='.$id .'&filename='.preg_replace('/[<>:"\/\\\|\?\*]/is', '', $download->c_background_pdf) .'&id=' . $id, FALSE, (int)$uri->isSSL()) . '">
