...
function addFlipRunningScripts($width, $height, $item, $isTmplComp)
{
	...
	foreach ($item->pages as $i => $page) {
		if (isset($page['page_image']) && !empty($page['page_image']))
		{
			...
			$largeIMG = COMPONENT_MEDIA_URL. 'images/'. ( $item->c_imgsub ? $item->c_imgsubfolder.'/' : '') . 'original-' . $item->c_id . '/' . str_replace(array('th_', 'thumb_'), '', $page['page_image']);
			if (!file_exists(COMPONENT_MEDIA_PATH . '/images/'. ( $item->c_imgsub ? $item->c_imgsubfolder.'/' : '') . 'original-' . $item->c_id . '/' . str_replace(array('th_', 'thumb_'), '', $page['page_image'])))
			{
				$largeIMG = $smallIMG;
			}
