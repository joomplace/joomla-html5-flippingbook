                            ...
                            foreach($pages as $i => $page){
                                ...
                                switch($i){
                                    ...
                                    case $bc+1 :
                                        ...
                                        $page_class .= ' fixed';
                                        $page_class .= ' p'.($pages_count+($double_page?$item->pages_count:0)-1);
                                        $page_number  = (($item->navi_settings)?($pages_count-1):'');
                                        break;
                                    case $bc+2 :
                                        $page_class .= ' cover-back';
                                        $page_class .= ' p'.($pages_count+($double_page?$item->pages_count:0));
                                ...
                                switch($i){
                                    ...
                                    default:
                                        ...
                                        $page_content = ($page['page_image'])?'<div class="paddifier"><img src="'.str_replace("\\", "/", JHtml::_('thumbler.generate', $page['page_image'], $page['id'].'_', json_encode(array('width' => $item->resolutions->width*($double_page&&($i!=2 && $i!=$bc)?2:1), 'height'=> $item->resolutions->height)), false)).'" /></div>':'<div class="paddifier"><div class="html-content"><div>'.$page['c_text'].((1)?'<span class="page-number">'.($page_number?($page_number+($pageCountModifier?$pageCountModifier:'')):'').'</span></div></div>':'').'</div>';
                                        $page_number = 0;
                                }
        ...
        var module = {
            ...
            plugins: function () {
                ...
                flipbook.turn({
                    ...
                    gradients: true,
                    duration: 1000,
                    pages: <?php echo $pages_count+($double_page?$item->pages_count:0); ?>,
                    when: {
                        turning: function(e, page, view) {
                            ...
                            book.parent().next().find('#goto_page_input').prop('placeholder',(page-<?php echo ($item->navi_settings)?0:1; ?>+parseInt(page_count_modifier))+' <?= JText::_( 'COM_HTML5FLIPPINGBOOK_PAGE_IS_OPENED');?>');
...
if($item->template->hard_wrapp){
    ...
    <style>
        .page-wrapper[page="1"],
        .page-wrapper[page="2"],
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"],
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"]{
        ...
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"] > div:first-child {
            margin: -2% -3.7% -2% 0px;
            padding: 2% 3.7% 2% 0px;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"] > div:nth-child(2) {
            margin: -2% 0px -2% -3.7%;
            padding: 2% 0px 2% 3.7%;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"] .p<?php echo $pages_count+($double_page?$item->pages_count:0); ?>{
            margin: -2% 0px -2% -3.7%;
            padding: 2% 0px 2% 3.7%;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"] .p<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>{
            margin: -2% -3.7% -2% 0px;
            padding: 2% 3.7% 2% 0px;
        }
        /* back-cover (like front-inside) */
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"] > div:first-child {
            margin: -2% 0px -2% -3.7%;
            padding: 2% 0px 2% 3.7%;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"] > div:nth-child(2) {
            margin: -2% -3.7% -2% 0px;
            padding: 2% 3.7% 2% 0px;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"] .p<?php echo $pages_count+($double_page?$item->pages_count:0); ?>{
            margin: -2% 0px -2% -3.7%;
            padding: 2% 0px 2% 3.7%;
        }
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"] .p<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>{
            margin: -2% -3.7% -2% 0px;
            padding: 2% 3.7% 2% 0px;
        }
        ...
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0)-1; ?>"] > div ,
        .page-wrapper[page="<?php echo $pages_count+($double_page?$item->pages_count:0); ?>"] > div {
            height:100%!important;
            width: 100%!important;
        }
