...
function print_locale_overlay_pdf_description() {
	...
	$print_locale_overlay_pdf_description="Download the book in PDF. Size: 3.3MB";
	return $print_locale_overlay_pdf_description;	
}

function print_locale_overlay_pdf_title() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_PDF_TITLE();	
	$print_locale_overlay_pdf_title="Download PDF";
	return $print_locale_overlay_pdf_title;		
}

function print_locale_overlay_print_description() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_PRINT_DESCRIPTION();	
	$print_locale_overlay_print_description="Print the book (in Letter or A4 size only)";
	return $print_locale_overlay_print_description;		
}

function print_locale_overlay_print_title() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_PRINT_TITLE();	
	$print_locale_overlay_print_title="Print Book";	
	return $print_locale_overlay_print_title;	
}

function print_locale_overlay_resume_yes() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_RESUME_YES();
	$print_locale_overlay_resume_yes="RESUME<br/>READING";	
	return $print_locale_overlay_resume_yes;		
}

function print_locale_overlay_resume_no() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_RESUME_NO();	
	$print_locale_overlay_resume_no="GO TO THE<br/>BEGINNING";	
	return $print_locale_overlay_resume_no;	
}

function print_locale_overlay_resume_description() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_RESUME_DESCRIPTION();
	$print_locale_overlay_resume_description="You can pick up from where you left off the last time, or start at the beginning. Do you want to:";
	return $print_locale_overlay_resume_description;			
}

function print_locale_overlay_resume_title() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_RESUME_TITLE();
	$print_locale_overlay_resume_title="RESUME READING?";	
	return $print_locale_overlay_resume_title;		
}

function print_locale_overlay_close() {
	// global $loc;
	// echo $loc->getLOCALE_OVERLAY_CLOSE();	
	$print_locale_overlay_close="CLOSE";
	return $print_locale_overlay_close;		
}
...
?>
		...
		<nav id="chapter-nav">
			...
			<ul>
				<?php 
				include_once ('php/includes/pages.php');
					...
					// $dataList  = ' data-title="'.'What is the Internet?'.'"';
					// $dataList .= ' data-subtitle="'.'subtitle'.'"';
					// $dataList .= ' data-article="'.'what-is-the-internet'.'"';
					// $dataList .= ' data-globalstartpage="'.'4'.'"';
					// $dataList .= ' data-globalendpage="'.'6'.'"';
					// $dataList .= ' data-numberofpages="'.'3'.'"';
					// $cnClass = 'what-is-the-internet';
					// $cnTitle = 'What is the Internet?';
					// $cnLink = '/what-is-the-internet';
					// $cnPages = '4-6';	
