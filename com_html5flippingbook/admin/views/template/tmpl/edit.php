...
JFactory::getDocument()->addScript(
	COMPONENT_ASSETS_URL . 'js/templatePreview.js'
);
...
if (!empty($this->item->fontsize))
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview { font-size: ' . $this->item->fontsize . '}'
	);
}
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview { font-family: '
		. PublicationTemplateFont::FontsList($this->item->fontfamily) . '}'
	);
}
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview p { margin: ' . $this->item->p_margin . ' 0}'
	);
}
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview p { line-height: ' . $this->item->p_lineheight . '}'
	);
}
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview { background-color: '
		. $this->item->page_background_color . '}'
	);
}
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview > div { background-color: '
...
{
	JFactory::getDocument()->addStyleDeclaration(
		'.template_preview > div { color: ' . $this->item->text_color . '}'
	);
}
?>
...
<form name="adminForm" id="adminForm" action="index.php" method="post"
	...
	<div id="j-main-container" class="span9 form-horizontal">
		...
		<?php echo JHtml::_(
			'bootstrap.startTabSet', 'myTab', array('active' => 'details')
		); ?>

		<?php echo JHtml::_(
			'bootstrap.addTab', 'myTab', 'details',
			JText::_('COM_HTML5FLIPPINGBOOK_BE_DETAILS', true)
		); ?>
		...
		<?php echo JHtml::_(
			'bootstrap.addTab', 'myTab', 'page-style',
			JText::_('COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_PAGESTYLE', true)
		); ?>
		<div class="control-group">
			<div class="control-label">
		...
		<div class="control-group">
			...
			<div class="controls">
				<?php echo JHtml::_(
					'select.genericlist', PublicationTemplateFont::P_margin(),
					'jform[p_margin]', '', 'value', 'text',
					$this->item->p_margin
				); ?>
			</div>
		</div>
		...
		<?php echo JHtml::_(
			'bootstrap.addTab', 'myTab', 'elements-display', JText::_(
			'COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_DISPLAY_ELEMENTS', true
		)
		); ?>
		<div class="control-group">
			<div class="control-label">
		...
		<?php echo JHtml::_(
			'bootstrap.addTab', 'myTab', 'preview',
			JText::_('COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_PREVIEW', true)
		); ?>
		<span class="label label-info"><?php echo JText::_(
				'COM_HTML5FLIPPINGBOOK_BE_TEMPLATES_PREVIEW_NEEDSAVE'
			); ?></span>
		<div class="template_preview">
			<div>
				<div>
					...
					<p>Joomla is an award-winning content management system
						...
						popular Web site software available. Best of all, Joomla
						is an open source solution that is freely available to
						everyone. </p>
					<h3>What's a content management system (CMS)?</h3>
					<p>A content management system is software that keeps track
						of every piece of content on your Web site, much like
						your local public library keeps track of books and
						stores them. Content can be simple text, photos, music,
						video, documents, or just about anything you can think
						of. A major advantage of using a CMS is that it requires
						almost no technical skill or knowledge to manage. The
						CMS manages all your content, so you don't have
						too. </p>
				</div>
				<div>
					<h1>I need to build a site for a client. How will Joomla!
						help me?</h1>
					<p>Joomla is designed to be easy to install and set up even
						if you're not an advanced user. Many Web hosting
						services offer a single-click install, getting your new
						site up and running in just a few minutes.</p>
					<p>Since Joomla is so easy to use, as a Web Designer or
						Developer, you can quickly build sites for your clients.
						Then, with a minimal amount of instruction, you can
						empower your clients to easily manage their own sites
						themselves.</p>
					<p>If your clients need specialized functionality, Joomla is
						highly extensible and thousands of extensions (most for
						free under the GPL license) are available in the Joomla
