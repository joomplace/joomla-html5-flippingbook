...
class HTML5FlippingBookControllerSample_Data extends JControllerLegacy
{
	...
	public function install()
	{
		...
		$query = "SELECT `id`" .
			...
			" WHERE `resolution_name` = '460 x 600'";
		$db->setQuery($query);
        $resolutionId = $db->loadResult();
		
		if (!$resolutionId)
		{
			$query ="INSERT INTO `#__html5fb_resolutions` (`id`, `resolution_name`, `height`, `width`) VALUES" .
				"(0, '460 x 600', 600, 462)";
			$db->setQuery($query);
			$db->execute();

            $resolutionId = $db->insertid();
		}
		...
		if (!$db->loadResult())
		{
			...
			$query = "INSERT INTO `#__html5fb_publication`" .
				" (`c_category_id`, `c_title`, `c_template_id`, `c_resolution_id`," .
				...
				" VALUES (" .
                $categoryId.", ".$db->quote($title).", 1, ".$resolutionId.", " .
			...
			foreach ($pages as $page)
			{
				$query = "INSERT INTO `#__html5fb_pages`" .
					" (`id`, `publication_id`, `page_title`, `ordering`, `c_enable_image`,`page_image`)" .
					" VALUES" .
					" (0, ".$publicationId.", ".$db->quote($page->title).", ".$pageOrder.", 1, ".$db->quote($page->image).")";
				$db->setQuery($query);
				$db->execute();
		...
		if (!$db->loadResult())
		{
			...
			foreach ($htmlFiles as $page)
			{
				...
				$query = "INSERT INTO `#__html5fb_pages`" .
					" (`id`, `publication_id`, `page_title`, `ordering`, `c_enable_image`,`c_text`)" .
					" VALUES" .
					" (0, ".$publicationId.", ".$db->quote('Page '.$pages).", ".$pageOrder.", 0, ".$db->quote($page).")";
				$db->setQuery($query);
				$db->execute();
		...
		JFactory::getApplication()->redirect('index.php?option='.COMPONENT_OPTION.'&view=publications', JText::_("COM_HTML5FLIPPINGBOOK_BE_SAMPLEDATA_INSTALLED"), 'message');
	}
}